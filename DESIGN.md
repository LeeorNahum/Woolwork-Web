...
version: alpha
name: Woolwork
description: A hand-stitched world of wool, felt, and yarn. Tokens mirror the Woolwork kit (woolwork.css v2.0.0), which is the wired token source for this repo.
colors:
  board: "#e3ddd0"
  cream: "#f6efdf"
  rose: "#e2707e"
...
The wired token source in this repository is `woolwork.css` (the kit, v2.0.0). This file is the reference those tokens follow; when the kit re-dyes, this file updates in the same change.
...
- **Buttons (`.btn-patch`):** felt patches with a stitched inset border, padded 14px vertically and 24px horizontally; hover lifts 1.5px, press settles 1.5px into a deepened inset shadow. Dye per instance with `--c`/`--t` token pairs. Compact actions use the same stitched patch material.
- **Cards (`.felt`, `.knit`):** felt or stockinette-knit patches; `.stitch` adds the sewn border, `.sew` the place-then-stitch arrival. The animated stitch reveals the exact final SVG dash nodes one at a time, with no mask or path replacement, so every visible dash keeps one weight and color through settlement.
- **Inputs (`.pocket`):** text fields are slits cut into the felt; selects are a single properly sized paper choice sheet, sized and aligned like the fields beside it, with opaque concertina paper options unfolding from its own bottom edge. Closing is immediate because edge-on reverse folds read as transparent bars over the next control. Focus wraps a rose thread ring.
...
- **Tables (`table.quilt`):** one continuous quilt surface whose four outer corner cells carry the rounding.
- **Theme state:** night mode is the URL state `theme=night`. The toggle updates the current URL and same-origin links, and the head restores it before paint so navigation keeps the chosen room without a flash.
