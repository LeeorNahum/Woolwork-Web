...
- **Inputs (`.pocket`):** text fields are slits cut into the felt; selects are paper choice slips tucked between the upper and lower lips of a felt cutout, with concertina paper options. Focus wraps a rose thread ring.
- **Checkboxes (`.sew-check`) and radios (`.snap-radio`):** four-hole buttons cross-stitched shut and snap fasteners.
- **Navigation and overlays:** tabs are hand-cut fabric labels resting on a stitched rail, separate from the content panel. Modals are felt notes clipped over the board; dropdowns are paper slips slotted behind their trigger; up to four toasts stack in a corner tray, each dismissable by its yarn cross or a swipe.
