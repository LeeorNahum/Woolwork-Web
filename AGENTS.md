...
- `og.png` is the shared social card (1200x630), a rendered snapshot of the hero. It is not auto-generated at deploy, so regenerate it whenever the hero changes (its wording, the bunting, the clips or other hardware, the logo dye, or the brand faces) and whenever the kit re-copy changes how the hero paints. Regenerate by rendering the hero at exactly 1200x630 with reduced motion (so reveals are settled and the bunting is drawn once), then overwrite `og.png`; keep the `og:image:width`/`height`/`alt` and `twitter:image` tags on both pages in step with it.
- Before committing a hero or kit-painting change, compare the freshly rendered 1200x630 hero with `og.png`. A changed clip, shadow, type treatment, stitch, or texture counts as a social-card change even when the hero markup and wording did not change.
